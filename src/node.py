...
class Node():
    ...
    def in_interval(self, key, start, end):
        """
        Returns True if key ∈ (start, end] in circular space
        """
        if start < end:
    ...
    def successor(self, key_value):
        ...
        if self.in_interval(key, self.previous_node.key, self.key):
            return ReferenceNode(self.ip, self.port)

        # otherwise forward to next
